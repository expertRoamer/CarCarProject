...
void loop()
{
	...
	if (path.length() == 0 || lastPath == 'B' || lastPath == 'A' || (path.length() > 0 && path[0] == 'B') || (path.length() > 0 && path[0] == 'A'))
	{
	...
	else
	{
		...
		else
		{
			...
			if (path[0] == 'B') driveKinematic(170, -turn);
			else driveKinematic(255, -turn);
		}
	}
...
void runTurn() {
	...
	double output1 = startVel1 - (startVel1 - endVel1) * elapsedTime1 / duration1;

	if (command == 'L')
	{
		drive(0, output1); // output1
		turningDir = -1;
	}
	else if (command == 'R')
	{
		drive(output1, 0); // output1
		turningDir = 1;
	}
	else if (command == 'B')
	{
		drive(-output, output); // 70
		turningDir = -1;
	}
	else if (command == 'A')
	{
